...
2. **Close the milestone — before the render, not after.** `pm feature review` / `pm feature done <fid> --review-record <path>` for each feature first (`pm milestone done X.Y.Z` refuses while any is live), and promote anything durable out of each `review.md` into `decisions.md` before deleting it (D11 fails a `done` grain that still has one). `pm milestone done X.Y.Z` **stamps `actual_date:`** with today's ISO date — close is the moment a milestone acquires one, and it is the date the render puts in `## vX.Y.Z — <date>`. Render first and the section comes out dateless, because the render is a pure function of the tree and never reaches for a clock. `check pm` at this point is the release gate: D16 on an empty log, D11 on a surviving `review.md`, D18 on a raw decision trail that was never collapsed to pointers.
