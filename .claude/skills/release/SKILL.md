...
2. **Close the milestone — before the render, not after.** `pm feature review` / `pm feature done <fid> --review-record <path>` for each feature first — point it at the feature's `decisions.md`, which is where a review's durable half belongs (`pm milestone done X.Y.Z` refuses while any feature is live). `pm milestone done X.Y.Z` **stamps `actual_date:`** with today's ISO date — close is the moment a milestone acquires one, and a date reconstructed later is a guess. `check pm` at this point is the release gate.
