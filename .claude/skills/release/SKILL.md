...
7. Report the consumer follow-up explicitly: each consumer bumps `DEVKIT_VERSION` in its Makefile (trail: `~/workspace/trail/Makefile`; nullbound: `~/workspace/nullbound/Makefile`), runs `install-* --diff` for what the release shipped and then decides PER FILE — `--force` is whole-set and has no per-file option, so it replaces every file that verb writes, including the ones the consumer deliberately edited (measured: trail's `verify.yml` is a two-job sharded workflow 177 lines from the installable, and nullbound's `auto-tag.yml` carries a path filter the installable does not). `--force` for a file the consumer has never touched; hand-apply the diff for one they have. Re-runs `pm init` once (a `.gitattributes` line a release added reaches an existing tree only that way), runs its gate set, commits the diff. Do NOT edit consumer repos from this session unless the user asks.
