...
2. Bump the README's two pin sites to `vX.Y.Z` in the same commit, both under "## Install — two pins": the `GODOT_DEVKIT_VERSION := vX.Y.Z` line in the Makefile snippet, and the `uvx --from "git+https://github.com/cdowin/godot-devkit@vX.Y.Z" godot-devkit install-runners` example. `DEVKIT_VERSION` beside it is agentic-sdlc's pin, so do not touch it. These are the strings consumers copy-paste; a stale pin there ships an old release to every new adopter.
