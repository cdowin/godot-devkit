...
- **the fresh game is a gate in both tiers** — the suite builds an empty Godot 4 project, runs `init`, asserts the written file set is exactly the documented one (cross-checked against the plan tables so the literal cannot rot), that a second run writes nothing, and dry-runs all 26 standard targets off the INSTALLED include with a before/after census proving the dry runs stayed dry. `make smoke` carries the half no dry run can reach: `init` in a scratch project and the REAL `make doctor`, splitting an init-owned failure (hooks unarmed) from a host gap (no gdlint) and saying loudly when `godot` is off PATH. Suite 1,034 tests + 3,673 subtests. (tests/test_fresh_project.py, tests/test_init_verb.py, tools/consumer_smoke.py)
