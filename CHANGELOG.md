...
- **`ci-semver-gate.yml` — versions of any length** — the gate read exactly three components and dropped the rest, so a hotfix `0.90.3` -> `0.90.3.1` compared as equal and was refused, and a letter suffix (`0.90.3a`) crashed the integer test — on that PR and on every later one, because main would then carry it. Now component-wise over the longer of the two, a missing component 0, a non-numeric component an explicit refusal. Probed 8 ways (bump / equal / lower / four-part / letter). Re-install with `install-ci --force`. (repo/installables/ci-semver-gate.yml)
