...
`check pm` and `pm validate`, and from nothing else. `pm status`, `pm list`, `pm get`,
`pm new` and `pm vocabulary --json` keep working, so you can read your own tree AND ask the tool
...
| `pm list [--status <s>[,<s>…]] [--owner <name>] [--milestone <id>]` | One tab-separated `<story-id>  <status>  <owner>  <feature-id>` per story, filtered. A view over facts — no ranking, no scoring, and deliberately **no `pm next`**: a verb that picks THE next thing is the tool having an opinion about your priorities. Measured on one consumer, `pm status` prints 165 lines and `pm list --status wip,review,blocked` prints the 2 that are open. Rows go to stdout so the output pipes; the census goes to stderr, so "matched nothing" and "scanned nothing" stay distinguishable |
