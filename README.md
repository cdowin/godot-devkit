...
| `pm ledger report [<milestone-id>] [--json]` | The milestone's raw rows, added up into its five questions, per story/feature/bug: spend, review yield, rework, escapes, overhead shape. Arithmetic only — sum, count, subtract, group — and never exits non-zero on a number |
