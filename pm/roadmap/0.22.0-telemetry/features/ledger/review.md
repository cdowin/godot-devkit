# Feature Review — 0.22.0/ledger

**Commit range:** `236af3f..HEAD` (`2250e76`, `31b9690`, `7bee4a0`, `3a55fd6` and the
`pm(0.22.0/ledger)` closes), plus the uncommitted working tree at review time.
...
**L6 — the milestone id `0.22.0` is already a released tag.** `origin/main` is at
`a9b4291`, one commit past `ffd8bc0 release: v0.22.0 — a scenario declares its
coverage: …`, and is opening `0.22.1`. This branch's `pm/roadmap/0.22.0-telemetry/`
is a second, different 0.22.0. `__init__.py` here reads `0.21.0` (bump-at-close, per
