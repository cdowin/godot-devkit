...
to date every hole rather than assume it. Nothing outside `/Users/cdowin/workspace/godot-devkit` was
...
- **Anything outside this checkout** — rule 8. No path under `~/workspace/trail` or
  `~/workspace/nullbound` was read or written, and no git write command was run anywhere.
