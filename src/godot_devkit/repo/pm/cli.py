...
def cmd_install_skills(cfg: model.PmConfig, args: list[str]) -> int:
    ...
    if collisions:
        listed = (collisions[0] if len(collisions) == 1 else
                  '\n' + '\n'.join(f'    {rel}' for rel in collisions))
        raise Refused(
            f'{listed} exist{"s" if len(collisions) == 1 else ""} and '
            f'{"was" if len(collisions) == 1 else "were"} not generated by '
            f'this tool — move your version aside, or pass --force. Nothing '
            f'was written; the whole install is refused, not just the first '
            f'colliding file.')
