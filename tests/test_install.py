...
def test_this_repo_carries_what_install_ci_produces():
    """The workflow this repo runs is the PINNED kit's, not a copy of ours.

    Until 0.25.0 this asserted verify.yml was byte-current with THIS package's
    installable: the shape shipped here first. It no longer does — this repo is
    a consumer of agentic-sdlc, whose `install-ci` wrote the file and whose
    `adopt` holds it current. What this test now refuses is the inverse fork:
    a workflow at that path that is this package's installable (or a hand-edit
    of it) shadowing the pinned one, invisible because it still looks
    installed. The file must be present — verify.yml runs `make milestone`,
    which this repo has — and must not be ours.
    """
    present = [rel for _, rel in install.PLANS['install-ci']
               if (REPO_ROOT / rel).is_file()]
    assert WORKFLOW in present, (
        f'{WORKFLOW} is not present in this repo — the one workflow that '
        f'runs its full gate, and the floor this test would otherwise pass over')
    for name, rel in install.PLANS['install-ci']:
        if rel not in present:
            continue
        text = (REPO_ROOT / rel).read_text(encoding='utf-8')
        assert text != install.body_of(name), (
            f'{rel} is this package\'s own installable — this repo takes it '
            f'from the pinned agentic-sdlc (`agentic-sdlc install-ci`)')
        assert 'GENERATED by agentic-sdlc' in text.splitlines()[0], (
            f'{rel} does not open with the pinned kit\'s marker')


def test_this_repo_carries_the_roles_it_runs_byte_current():
    """PARTIAL-roster, from the PINNED kit.

    This package runs its own SDLC with the verification pair; the roles it
    does not run are legitimately absent. Both files are `agentic-sdlc
    install-agents`'s output since 0.25.0, and `agentic-sdlc adopt` is what
    holds them byte-current. What is refused here is the inverse fork: a file
    at a roster path that is THIS package's installable, or an edit of it,
    shadowing the pinned one under the same name.
    """
    present: list[str] = []
    for name, rel in install.PLANS['install-agents']:
        ...
        if target.is_file():
            ...
            assert text != install.body_of(name), (
                f'{rel} is this package\'s own installable — this repo takes '
                f'it from the pinned agentic-sdlc (`agentic-sdlc install-agents`)')
            assert 'GENERATED by agentic-sdlc' in text, (
                f'{rel} does not carry the pinned kit\'s marker')
    # The floor: a repo that stops carrying the pair has stopped running the
    # contract, and this test would otherwise pass vacuously.
    for rel in AGENTS[:2]:
