...
def test_a_declared_toolchain_reaches_the_workflow_before_the_gate():
    ...
    # Action-input KEYS are quoted for the same reason values are — see
    # `test_a_yaml_1_1_boolean_word_survives_as_an_action_input_name`.
...
def test_a_yaml_1_1_boolean_word_survives_as_an_action_input_name():
    """The defect: `_key` had a bare-key fast path matching `[A-Za-z0-9_-]+`.

    A YAML 1.1 reader — PyYAML, and most of what reads a workflow — resolves a
    bare `on`, `yes`, `123` or `0x1f` to a boolean, an integer or null. Two
    inputs named `on` and `yes` both became `True` and COLLIDED into one key,
    while every VALUE in the same file was already quoted against exactly this.
    """
    yaml = pytest.importorskip('yaml')
    names = ('on', 'off', 'yes', 'no', 'y', 'n', 'true', 'false', 'null',
             '123', '0x1f', '1_000', 'version')
    inputs = ', '.join(f'{name} = "v-{i}"' for i, name in enumerate(names))
    body = _workflow_for('[[ci.setup]]\n'
                         'uses = "some/action@v1"\n'
                         f'with = {{ {inputs} }}\n')
    got = yaml.safe_load(body)['jobs']['verify']['steps'][2]['with']
    assert got == {name: f'v-{i}' for i, name in enumerate(names)}, got


def test_every_emitted_key_is_quoted():
    """The property, stated once: a key is a string, like every value."""
    from godot_devkit.repo.install import _key
    for name in ('on', 'yes', '123', 'version', 'include-templates'):
        assert _key('where', name) == f'"{name}"'


# --- the all-or-nothing property, past the collision case ---------------------
# It covered collisions only. A destination that is a directory, one that is
# read-only, one holding bytes that are not UTF-8 — each raised a traceback, and
# two of them raised it AFTER the first file had already been written. A stack
# trace names no repair and its "nothing was written" is a claim nobody made.
def _refuse(command: str, *argv: str) -> tuple[int, str]:
    buffer = io.StringIO()
...
def test_a_destination_that_is_a_directory_is_a_refusal_not_a_traceback():
    with repo() as root:
        ...
        code, out = _refuse('install-agents')
    assert code == 1, out
...
def test_a_read_only_destination_is_a_refusal_that_writes_nothing():
    with repo() as root:
        ...
        for rel in AGENTS:
            (root / rel).write_text('GENERATED by godot-devkit — stale\n',
                                    encoding='utf-8')
        ...
        # The first entry is the one that proves it: it WOULD have been
        # rewritten, and a refusal decided up front leaves it alone.
        assert (root / AGENTS[0]).read_text(encoding='utf-8') != keep
        assert 'stale' in (root / AGENTS[0]).read_text(encoding='utf-8')


def test_a_non_utf8_destination_is_a_collision_and_force_overwrites_it():
    """Undecodable bytes cannot carry the generated header, so the file is
    somebody else's — the same answer as any other foreign file, not a crash."""
    with repo() as root:
        (root / AGENTS[1]).parent.mkdir(parents=True, exist_ok=True)
        (root / AGENTS[1]).write_bytes(b'\xff\xfe\x00not utf-8')
        code, out = _refuse('install-agents')
        assert code == 1, out
        assert 'not generated by this tool' in out, out
        assert not (root / AGENTS[0]).exists(), out
        code, out = _refuse('install-agents', '--force')
        assert code == 0, out
        assert 'GENERATED by godot-devkit' in (
            root / AGENTS[1]).read_text(encoding='utf-8')
