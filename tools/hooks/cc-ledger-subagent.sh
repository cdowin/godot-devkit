...
# dispatch. Reads the event JSON on stdin, copies the agent's transcript path,
# session id, agent id and agent type into `pm ledger record`, and exits 0. It
...
# ARGS, so the wiring is proven without a PM tree.
self_test_payload() {
	python3 -c '
import json, sys
